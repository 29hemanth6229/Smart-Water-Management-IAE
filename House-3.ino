...
/* Because this loop may not complete in exactly 1 second intervals we calculate the number of milliseconds that have passed since the last execution and use
    ...
    // Sending the values to ThingSpeak
      if(Firebase.ready() && signupOK){
        Firebase.RTDB.setFloat(&fbdo,"Apartment or Colony/House-3 Volume in Litres",totalLitres);
        Firebase.RTDB.setFloat(&fbdo,"Apartment or Colony/House-3 Amount in Rupees",bill);
        }

      if (Firebase.RTDB.getString(&fbdo,"Apartment or Colony/House-3 Valve:")){
